...
def plot_distribution(i:int, name:str,data_dist:pd.DataFrame,axis:Axes):
    plot = data_dist.plot.hist(y="Total Request Count",ax=axis[i, 2])

    plot.set_facecolor('#B0C4DE')
    plot.set_title(f"{name} request distribution")
...
def generate_report(csv_file_prefix:str) -> Figure:
    ...
    for i,name in enumerate(names):
        ...
        data_dist = _data[["Total Request Count", "Total Max Response Time", "50%","66%","75%","80%","90%","95%","98%","99%","99.9%","99.99%","100%"]]
